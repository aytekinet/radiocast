...
export const StationCard: React.FC<StationCardProps> = React.memo(({
  ...
  return (
    <div
      ...
      className={`group relative rounded-2xl p-3 border transition-all duration-200 ease-out flex flex-col justify-between cursor-pointer h-[152px] min-h-[152px] max-h-[152px] ${
        isCurrentStation
    ...
    >
      {/* Top Header: Logo + Actions (Favorite + Playlist) */}
      <div className="flex-1 flex flex-col justify-between min-w-0">
        <div>
            ...
            <div className="relative shrink-0">
              {station.favicon && !imgError ? (
                <img
                  ...
                  className="w-10 h-10 rounded-xl object-contain bg-zinc-100 dark:bg-zinc-950 p-1 border border-zinc-200 dark:border-zinc-800 shadow-inner"
                />
              ) : (
                <div className="w-10 h-10 rounded-xl bg-amber-500/10 dark:bg-zinc-800 border border-amber-500/30 dark:border-zinc-700/80 flex items-center justify-center text-amber-500">
          ...
          <div className="my-1.5 min-w-0">
            <h3 className="font-bold text-xs sm:text-sm text-zinc-900 dark:text-zinc-100 group-hover:text-amber-500 transition-colors line-clamp-1 leading-snug">
          ...
          <div className="flex flex-wrap items-center gap-1 my-1">
            {station.countrycode && (
              <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700/60">
                {station.countrycode}
              </span>
            )}
            {tagsList.map((tag, idx) => (
              <span
                key={idx}
                className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-zinc-100/80 dark:bg-zinc-800/80 text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700/40 truncate max-w-[90px]"
              >
                #{tag.trim()}
              </span>
            ))}
      ...
      {addedNotice && (
        <div className="text-[10px] text-amber-600 dark:text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/30 mb-2 font-medium animate-pulse">
          ✓ {addedNotice} listesine eklendi
        </div>
      )}

      {/* Footer: Bitrate Badge & Play Button */}
      <div className="flex items-center justify-between pt-2 border-t border-zinc-200 dark:border-zinc-800/80 mt-1">
        <div className="flex items-center space-x-1.5 text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">
          {station.bitrate > 0 && (
        ...
        <button
          ...
          className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all shadow-md active:scale-90 ${
